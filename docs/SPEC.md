...
- **バックグラウンド再生はキープアライブ再生で維持する（実機の試行錯誤の結論）。**

  Android Chrome は「メディアを再生していないページ」を、画面ロックやアプリ切り替えで凍結する。Web Audio は音をその場で合成しているだけなので、Chrome から見るとメディアを再生していないことになり、音が止まる。

  試したこととその結果:

  | 試したこと | 結果 |
  |---|---|
  | 素の `ctx.destination` | 止まる。ロック画面に通知も出ない |
  | `MediaStreamAudioDestinationNode` → `<audio>` | **効果なし。** Chrome は MediaStream をメディアセッションの対象にしない（通知が出ず、やはり停止）。位相は保たれることを実測（分離 91.5 dB / 112.1 dB）したが、目的を果たさないので不採用 |
  | PWA としてインストール（standalone） | 効果なし。「アプリとして起動」でも停止 |
  | **実ファイルを `<audio>` でループ再生（採用）** | Chrome が「メディアを再生中のページ」と認識する経路に乗せる |

  実装（`keepalive.ts`）:
  - 極小レベル（−55 dBFS）のノイズを実行時に WAV として生成し、`<audio loop>` で鳴らす。**デジタル無音にはしない**——非可聴と判定されるとメディアセッションが張られず、仕組みが成立しないため。本編（−25 dBFS 前後）より 30 dB 低いので実質聞こえない。
  - 音そのものは従来どおり `ctx.destination` に直接出す。経路を最も単純に保ち、位相に手を触れない。
  - 要素は DOM に入れる（文書に属さないメディア要素を「再生中のメディア」として扱わないブラウザがある）。
  - `play()` の成功を待ってから採用し、失敗時は設定画面に理由を出す。
  - 一時停止では**状態を更新してから要素を止める**。順序を逆にすると、要素の `pause` イベントを「外部からの停止」と誤認して再帰する。
  - 音声アセットは配布物に増えない（実行時生成なので「音源ファイルを持たない」方針は保たれる）。

  **キープアライブは Web Audio に取り込んで出力を 1 本にまとめる（既定）。**
  別々の出力ストリームのままだと、実機（Android + Bluetooth）で
  **キープアライブのループ周期ちょうど（30 秒）ごとに音が途切れた**。
  ループの再開そのものが出力を一瞬止めていたことになる。
  `createMediaElementSource` で Web Audio に取り込むと出力が 1 本になり、
  ループの影響が出力に現れなくなる。

  この加算がビートを損なわないことは実測で確認済み:

  | | 左のピーク | 右のピーク | Δf | 312 Hz の左右分離 |
  |---|---|---|---|---|
  | まとめない | 312 Hz | 328 Hz | 16.000 | 122 dB |
  | まとめる | 312 Hz | 328 Hz | **16.000** | 84 dB |
  | 参考: 環境音そのもの（−28 dBFS） | | | | **62 dB** |

  加算は線形なので搬送波の位相関係に触れず、**Δf はまったく変化しない**。
  分離は下がるが、**アプリ本来の環境音のほうが影響が大きい**（62 dB）ので、
  キープアライブは実質的に無視できる。

  取り込みに失敗する環境のために設定で切り替えられるようにしてある。
- **Screen Wake Lock API** で画面スリープを抑止（対応環境のみ、設定でオフ可）。バックグラウンド再生が効く環境では不要なので、電池のために切ってよい。
