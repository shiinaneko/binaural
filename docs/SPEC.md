...
- **バックグラウンド再生は `<audio>` 要素経由で維持する。** 素の Web Audio（`ctx.destination`）は、Android Chrome で画面をロックしたりアプリを切り替えると止まる（実機で確認）。そこでリアルタイム再生では `MediaStreamAudioDestinationNode` → `<audio>` 要素に流し、ブラウザから「メディア再生」として扱わせる。ロック画面の MediaSession とも自然に噛み合う。
  - **この経路が位相を壊さないことを実測で確認済み**：左 312.01 Hz / 右 328.13 Hz（FFT ビン幅 1.46 Hz）、チャンネル分離 91.5 dB / 112.1 dB。位相が崩れればこのアプリは意味を失うので、採用前に必ず測ること。
  - `createMediaStreamDestination` が無い環境や `play()` が拒否された場合は `ctx.destination` にフォールバックする。実際に使われている経路は設定画面に表示し、実機での切り分けができるようにする。
  - 書き出し（`OfflineAudioContext`）は常に `ctx.destination` のまま。
